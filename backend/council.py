...
def _is_trading_query(
    ...
    false_positive_indicators = ['model', 'llm', 'vision', 'openrouter', 'gemini', 'claude', 'gpt', 'qwen', 'chart image', 'tradingview chart analysis', 'best models']
    fp_hits = sum(1 for ind in false_positive_indicators if ind in query_lower)

    # Require at least 4 trading indicators AND no strong non-trading context
    if hit_count >= 4 and fp_hits < 2:
        logger.info(
    ...
    if hit_count >= 4:
        logger.warning(
            "Potential false positive trading detection (%d hits) but "
            "non-trading context detected (%d fp indicators). Skipping trading enrichment.",
            hit_count, fp_hits
        )
    return False
# ---------------------------------------------------------------------------
...
async def stage3_synthesize_final(
    ...
    # Extra safeguard: if the query is about AI models / vision / platforms, force non-trading
    query_lower = user_query.lower()
    if any(term in query_lower for term in ['model', 'models', 'vision', 'openrouter', 'gemini', 'claude', 'gpt', 'qwen', 'llm', 'best value', 'tradingview charts images']):
        if is_trading:
            logger.warning("Forcing non-trading mode for AI model selection query despite keyword hits.")
            is_trading = False
